...
### 6.1 [x] Consider renaming `charm.Chart` or `pict.Chart` to resolve name collision
...
**Decision:** Accept the status quo for 0.5.0. Renaming either `charm.Chart` or
deprecated `pict.Chart` would be a breaking change, and the collision resolves
naturally if the pict API is removed in a future major version.

### 6.2 [x] Consider adding PDF export format
...
**Action:** Added PDF export using Apache PDFBox 3.0.1. The implementation
reuses the existing SVG-to-image render path and embeds the rendered image on a
single PDF page. `ExportFormat.PDF`, `ChartToPdf`, `Chart.writeTo(...pdf...)`,
and `PlotGrid.writeTo(...pdf...)` now mirror the existing PNG/JPEG/SVG APIs.
