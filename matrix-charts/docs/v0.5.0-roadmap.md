...
# TODO
NumberCoercionUtil in matrix-charts seems redundant as it is cuplicating similar functionality from matrix-core ValueConverter. Investigate if it is possible to remove it and replace it with ValueConverter from matrix-core to keep things DRY (possibly updating ValueConverter with additional functionality to enable the switch over).
