...
class GeomBar extends Geom {
  ...
  void render(G group, Matrix data, Aes aes, Map<String, Scale> scales, Coord coord, RenderContext ctx) {
    ...
    if (coord instanceof CoordRadial) {
      renderRadialWithContext(group, data, aes, scales, coord as CoordRadial, ctx)
      return
    }

    if (coord instanceof CoordPolar) {
      renderPolarWithContext(group, data, aes, scales, coord as CoordPolar, ctx)
      return
    }
    ...
    data.each { row ->
      ...
      GeomUtils.applyAttributes(rect, ctx, 'bar', 'gg-bar', elementIndex)
      elementIndex++
    }
  }

  private void renderRadialWithContext(G group, Matrix data, Aes aes, Map<String, Scale> scales,
                                        CoordRadial coord, RenderContext ctx) {
    String xCol = aes.xColName
    String yCol = data.columnNames().contains('count') ? 'count' : aes.yColName
    String fillCol = aes.fillColName
    Scale fillScale = scales['fill'] ?: scales['color']
    ...
    for (Map.Entry<Object, List<Row>> entry : groups.entrySet()) {
      ...
      rows.eachWithIndex { row, int rowIdx ->
        ...
        // Apply CSS attributes
        GeomUtils.applyAttributes(path, ctx, 'bar', 'gg-bar', elementIndex)
        elementIndex++
      }
    }
  }

  private void renderPolarWithContext(G group, Matrix data, Aes aes, Map<String, Scale> scales,
                                       CoordPolar coord, RenderContext ctx) {
    String xCol = aes.xColName
    String yCol = data.columnNames().contains('count') ? 'count' : aes.yColName
    String fillCol = aes.fillColName
    Scale fillScale = scales['fill'] ?: scales['color']
