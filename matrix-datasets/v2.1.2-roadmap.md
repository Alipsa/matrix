...
### 2.1 [ ] Add null safety to mapDataSet() method
**Severity:** MEDIUM
...
### 2.2 [ ] Use consistent Matrix naming approach
**Severity:** LOW
...
### 2.3 [ ] Replace generic Exception with FileNotFoundException
**Severity:** LOW
...
### 2.4 [ ] Use string interpolation instead of concatenation
**Severity:** LOW
...
### 3.2 [ ] Add edge case tests to DatasetTest
**Severity:** LOW
...
### 4.1 [ ] Update README.md version compatibility table
**Severity:** LOW
...
Add a method similar to `fromUrl()` for local files:
```groovy
static Matrix fromFile(String filePath, String delimiter = ',', String stringQuote = '') {
  Matrix.builder().data(FileUtil.checkFilePath(filePath), delimiter, stringQuote).build()
}
```

---

### 5.2 [ ] Add Logger implementation
Per AGENTS.md guidelines, add logging support using `se.alipsa.matrix.core.util.Logger` for error conditions and debugging.

---

### 5.3 [ ] Refactor mapData() to use Map lookup
