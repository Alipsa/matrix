...
### 2.2 [ ] Replace println with proper logging
- **Files**: `Bq.groovy` (13 instances), test files
- **Issue**: Direct console output makes it impossible to control logging in production
- **Solution**:
  - Add SLF4J dependency (already available via BigQuery client)
  - Replace `println` with `log.info()` or `log.debug()`
  - Add logger: `private static final Logger log = LoggerFactory.getLogger(Bq.class)`
- **Instances**:
  - Lines 213, 301, 308, 346, 363, 468, 524, 534, 562, 564, 580, 582
- **Effort**: 30 minutes

### 2.3 [ ] Fix overly broad exception catching
...
### 2.4 [ ] Refactor the insert method
- **File**: `src/main/groovy/se/alipsa/matrix/bigquery/Bq.groovy`
...
### 4.5 [ ] Make sync vs async query execution configurable
- **File**: `src/main/groovy/se/alipsa/matrix/bigquery/Bq.groovy`
- **Task**: Allow users to choose between sync (emulator compatible, 10 GB limit) and async (production, no limit) query execution
- **Solution**: Add constructor parameter or method flag to toggle execution mode
- **Benefits**:
  - Users can opt into async for production workloads without code changes
  - Tests can continue using sync mode with emulators
  - No breaking changes (sync remains default)
- **Effort**: 2-3 hours (requires refactoring execute() and query() methods)
...
- [ ] All tests passing (including external tests with valid credentials)
...
- **Priority 1 (Release Blockers)**: 30 minutes
- **Priority 2 (High Priority)**: 6-8 hours
- **Priority 3 (Medium Priority)**: 3-4 hours
- **Pre-Release Tasks**: 2 hours
- **Total**: 11-14 hours for production-ready release
...
2. **Quality release (1-2 weeks)**: Complete Priority 1 + Priority 2
   - Pros: Production-ready with proper testing and logging
   - Cons: Takes longer
