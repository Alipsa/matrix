...
class FExcelReader implements SpreadsheetReader  {
    ...
    int findRowNum(String sheetName, int colNumber, String content) {
        Sheet sheet = workbook.findSheet(sheetName).orElse(null)
        return findRowNum(sheet, colNumber, content)
    }
    ...
    static int findColNum(Sheet sheet, int rowNumber, String content) {
      ...
      FExcelValueExtractor ext = new FExcelValueExtractor(sheet)
    ...
    int findLastRow(int sheetNum) {
        findLastRow(workbook.getSheet(sheetNum -1).orElse(null))
    }

    @Override
    int findLastRow(String sheetName) {
        findLastRow(workbook.findSheet(sheetName).orElse(null))
    }
    ...
    int findLastCol(int sheetNum) {
        findLastCol(workbook.getSheet(sheetNum -1).orElse(null))
    }

    @Override
    int findLastCol(String sheetName) {
        findLastCol(workbook.findSheet(sheetName).orElse(null))
    }
