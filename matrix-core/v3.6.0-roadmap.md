...
3.1 [x] Clarify `split(int chunkSize)` semantics - Renamed parameter to `numChunks`, rewrote implementation to evenly distribute rows using base size + remainder algorithm, fixed validation to allow numChunks == rowCount(), updated docs with correct examples (Matrix.groovy:2243-2267)
3.2 [x] Align `dropEmptyColumns` behavior with docs - Now uses `containsValues()` to properly check for blank strings (Matrix.groovy:1978-1988)
...
3.8 [x] Run tests: `./gradlew :matrix-core:test` - **SUCCESS (199 tests, 199 passed, 0 failed, 0 skipped)**
