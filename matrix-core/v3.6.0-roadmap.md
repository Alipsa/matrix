...
**Impact:** API confusion,易错 mistakes
...
1.1 [ ] Fix `Matrix.rowIndices()` to execute criteria and add regression test for `removeEmptyRows` (Matrix.groovy:2139)
1.2 [ ] Fix `MatrixAssertions` to throw exceptions in `assertNotEquals`/`assertContentNotEquals` (MatrixAssertions.groovy:17)
1.3 [ ] Rewrite `Matrix.dropExcept(int...)` and `Grid.dropExcept()` to avoid index-shift bugs; add coverage (Matrix.groovy:953, Grid.groovy:930-962)
1.4 [ ] Add division-by-zero check in `Stat.variance()` (Stat.groovy:761)
1.5 [ ] Fix `MatrixBuilder` Path.contains() bug - use filename string instead of Path (MatrixBuilder.groovy:360)
1.6 [ ] Update `diff` and `checkValues` to handle nulls explicitly without treating `0` as false; add tests (Matrix.groovy:895,1111)
1.7 [ ] Replace `Arrays.sort(indexes)` with `indexes.sort()` in `removeRows(List)` and test unsorted removal (Matrix.groovy:1949)
1.8 [ ] Run tests: `./gradlew :matrix-core:test`

### Phase 2: Code Quality & Refactoring (Priority 2)
2.1 [ ] Optionally add explicit return to `Grid.toString()` for clarity (Grid.groovy:165)
2.2 [ ] Refactor `ClassUtils.nearest()` to use lookup table or priority algorithm (ClassUtils.groovy:96-188)
2.3 [ ] Extract common null-handling in Column arithmetic operations to `applyOperation()` method (Column.groovy:64-224)
2.4 [ ] Refactor ValueConverter for better modularity (#27)
2.4.1 [ ] Unify duplicate cache management methods (ValueConverter.groovy:504-520)
2.4.2 [ ] Consider organizing conversions by target type families (numeric, temporal, string converters)
2.5 [ ] Remove unused `jdk.internal.reflect.Reflection` import (ClassUtils.groovy:4)
2.6 [ ] Add null-check handling in `Stat.addNumericSummary()` callers (Stat.groovy:74)
2.7 [ ] Fix `ListConverter.convert()` assignability check to `type.isAssignableFrom(it.class)` (ListConverter.groovy:23)
2.8 [ ] Fix opentest4j dependency scope from `implementation` to `testImplementation` (build.gradle:43)
2.9 [ ] Consider refactoring `getAt/putAt` dispatching methods for better maintainability (#25)
2.10 [ ] Run tests: `./gradlew :matrix-core:test`

### Phase 3: API & Documentation (Priority 3)
3.1 [ ] Clarify `split(int chunkSize)` semantics and update implementation/docs/tests (Matrix.groovy:2240)
3.2 [ ] Align `dropEmptyColumns` behavior with docs or update docs; add tests (Matrix.groovy:1968)
3.3 [ ] Standardize logging framework (#20)
3.3.1 [ ] Add SLF4J facade dependency to build.gradle
3.3.2 [ ] Replace System.err.println with SLF4J logger calls (MatrixBuilder.groovy:481, Stat.groovy:74)
3.4 [ ] Standardize null/empty string handling in ValueConverter methods (part of #27)
3.5 [ ] Document deprecated methods: removal timeline, performance implications, behavioral differences
3.6 [ ] Fix JavaDoc typos and add missing documentation (Grid.groovy:80, ClassUtils.groovy:81, MatrixBuilder.rows():120, Grid.isValid():286)
3.7 [ ] Consider MatrixPrinter dependency inversion for better separation of concerns (#26)
3.8 [ ] Run tests: `./gradlew :matrix-core:test`
...
## Success Criteria
- [ ] All critical bugs (Phase 1) fixed with tests
- [ ] Test suite passes: `./gradlew :matrix-core:test`
- [ ] No regression in existing functionality
- [ ] Code coverage maintained or improved (measured via JaCoCo)
- [ ] Static analysis passing (CodeNarc/SonarLint)
- [ ] Documentation updated for all changes
- [ ] Dependency management cleaned up (SLF4J standardized, opentest4j scope corrected)
