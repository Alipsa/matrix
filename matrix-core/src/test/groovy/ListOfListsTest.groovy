...
class ListOfListsTest {
  ...
  void testConvertToBigDecimal() {
    ...
    def baz = convert(foo2, 0, BigDecimal, format)
    assertEquals(17.6, sum(baz, 0))

    def baz2 = convert(foo2, [0, 1, 2], BigDecimal, format)
  ...
  void testCastToDouble() {
    ...
    def bar = convert(foo, 0, Double, null)
    ...
    def format = DecimalFormat.getInstance(Locale.GERMANY)
    def baz2 = convert(foo2, 0..2, Double, format)
    assertEquals(7.01, sum(baz2, [0, 1])[1], 1e-9)

    def baz3 = convert(foo2, 0, Double, format)
    def baz4 = convert(foo2, 0, Double, format)
