...
class StatTest {
  ...
  @Test
  void testMedians() {
    // Test medians(List<List<?>>, Integer) - single column variant
    def matrix = [
        [1, 2, 3],
        [1.1, 1, 0.9],
        [null, 7, "2"]
    ...
    def m1 = medians(matrix, 1)
    assertEquals(1, m1.size())
    assertEquals(2 as BigDecimal, m1[0])

    def m2 = medians(matrix, 2)
    assertEquals(1, m2.size())
    assertEquals(1.95g, m2[0])

    // Test medians(Matrix, String) - single column name variant
    def table = Matrix.builder().columnNames(["v0", "v1", "v2"]).rows(matrix).build()

    def m3 = medians(table, "v1")
    assertEquals(1, m3.size())
    assertEquals(2 as BigDecimal, m3[0])

    def m4 = medians(table, "v2")
    assertEquals(1, m4.size())
    assertEquals(1.95g, m4[0])
  }
  ...
  void testSd() {
    ...
    assertEquals(1.365039681962885, sd(matrix[0]))
    assertEquals(1.365039681962885, sdSample([0.3, 2, 3]))
    def s = sd(matrix, [1, 2])
    assertEquals(3.214550253664318, s[0])
    assertEquals(1.484924240491750, s[1])
