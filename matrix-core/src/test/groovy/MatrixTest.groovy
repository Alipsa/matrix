...
class MatrixTest {
  ...
  @Test
  void testOrOperatorBasic() {
    def m = Matrix.builder().data(
        a: [3, 1, 2],
        b: [6, 4, 5]
    ).types(Integer, Integer).build()

    def result = m | { it.orderBy('a') }
    assertEquals([1, 2, 3], result.column('a') as List)
  }

  @Test
  void testOrOperatorChained() {
    def m = Matrix.builder().data(
        a: [3, 1, 2],
        b: [6, 4, 5]
    ).types(Integer, Integer).build()
  ...
  @Test
  void testPipeMixedFilterAndMutate() {
    def m = Matrix.builder().data(
        a: [1, 2, 3],
        b: [10, 20, 30]
    ).types(Integer, Integer).build()

    def result = m | { it.subset('a', { it > 1 }) } | { it.apply('b') { it * 2 } }
    assertEquals(2, result.rowCount())
    assertEquals([40, 60], result.column('b') as List)
  }

  @Test
  void testPipeReturnsNonMatrix() {
    def m = Matrix.builder().data(
        a: [1, 2, 3]
    ).types(Integer).build()

    def result = m.pipe { it.rowCount() }
