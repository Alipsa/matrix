...
- Numeric cell values now compare equal to their equivalent single-character String cells in both directions. Previously the Number-first direction reported a difference.
- `Row` now has content-based `equals()` and `hashCode()` behavior consistent with Groovy value comparison, including numerically equivalent values and nested collections. Rows are mutable views, so do not use a Row as a key in a hash-based collection while mutating its values.
...
- Column.minus can now handle nulls so `matrix['columnA'] - null` now works the same as `matrix['columnA'].removeNulls()`
