...
**Recommendation:**
- Replace GStrings without interpolation (e.g., `"${value}"` → `'value'` or `"value"`) with plain strings
...
- CodeNarc violations in ValueConverter.groovy and MatrixBuilder.groovy are currently excluded due to Groovy 5 switch expression syntax not yet supported by the documented baseline (CodeNarc 3.5.0)
- Action: Periodically verify whether a newer CodeNarc version supports Groovy 5 switch expressions and, once adopted, remove the temporary exclusions for ValueConverter.groovy and MatrixBuilder.groovy
