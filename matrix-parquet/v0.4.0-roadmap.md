...
### 6. Add class-level documentation
Both main classes lack class-level GroovyDoc comments.

6.1 [ ] Add class-level GroovyDoc to `MatrixParquetWriter` describing its purpose and usage
6.2 [ ] Add class-level GroovyDoc to `MatrixParquetReader` describing its purpose and usage

## Feature Enhancements

### 7. Add timezone support for timestamps
Currently all timestamps use `ZoneId.systemDefault()`, which can cause data loss when reading files written on systems with different timezones.

7.1 [ ] Add optional timezone parameter to writer methods
7.2 [ ] Add optional timezone parameter to reader methods
7.3 [ ] Document timezone handling behavior in GroovyDoc
7.4 [ ] Add test for timezone-aware timestamp handling
