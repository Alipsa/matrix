...
### 8. Improve error messages (COMPLETED)
Error handling existed but messages were not descriptive enough.

**Resolution:**
- Improved list field type error: now includes field name, expected type, actual type, and truncated value
- Improved map field type error: now includes field name, expected type, actual type, and truncated value
- Added explicit BigDecimal precision overflow check with descriptive message including value, field name, bytes required vs allowed, precision/scale, and suggestion to use `inferPrecisionAndScale=true`
- Added `truncateForError()` helper method to prevent excessively long error messages
- Updated `testMatrixParquetPrecisionOverflow` test to verify new error message
- Tests verified with `./gradlew :matrix-parquet:test`

8.1 [x] Improve `IllegalArgumentException` message for list field type error
8.2 [x] Improve `IllegalArgumentException` message for map field type error
8.3 [x] Add descriptive error when precision overflow occurs during BigDecimal write

## Performance Optimizations

### 9. Cache reflection metadata for struct handling (COMPLETED)
The `buildStructType()` method used Introspector on every call without caching PropertyDescriptors, which could be slow for large matrices with complex objects.

**Resolution:**
- Added `PROPERTY_DESCRIPTOR_CACHE` (ConcurrentHashMap) to `MatrixParquetWriter` for caching PropertyDescriptor lists by class
- Added `getPropertyDescriptors(Class)` helper method that uses `computeIfAbsent` for thread-safe caching
- Updated `buildStructType()` and `toStructMap()` to use cached PropertyDescriptors
- Added `CLASS_CACHE` (ConcurrentHashMap) to `MatrixParquetReader` for caching Class.forName results
- Added `getCachedClass(String)` helper method for thread-safe class caching
- Updated `parseTypeString()` to use cached class lookups
- Tests verified with `./gradlew :matrix-parquet:test`

9.1 [x] Cache PropertyDescriptor results for repeated struct type inference
9.2 [x] Cache `Class.forName()` results during reading

## Documentation Improvements

### 10. Document known limitations (COMPLETED)
The readme.md needed documentation of edge cases and limitations.

**Resolution:**
Completely rewrote `readme.md` to include:
- Updated installation instructions (v0.4.0, Groovy 5.0.3)
- Removed references to deprecated MatrixParquetIO
- Added comprehensive "Supported Data Types" table with Parquet mappings
- Added "BigDecimal Precision" section with three approaches and examples
- Added "Nested Structures" section with examples and limitations
- Added "Timezone Handling" section with examples
- Added "Known Limitations" section covering:
  - Memory usage (in-memory structure, RAM requirements, mitigation strategies)
  - JDK compatibility (JDK 21 max due to Hadoop)
  - Timestamp precision (microseconds for DateTime, milliseconds for Time)
  - Type metadata behavior for external files
- Added "Technical Notes" section on thread safety and performance tips
- Updated 3rd party libraries table

10.1 [x] Document that Matrix is an in-memory structure (no streaming support)
10.2 [x] Document timezone handling behavior
10.3 [x] Document BigDecimal precision/scale requirements
10.4 [x] Document nested structure support and limitations

## Test Coverage Improvements

### 11. Add edge case tests (COMPLETED)

11.1 [x] Add test for empty Matrix write/read round-trip
11.2 [x] Add test for Matrix with only null values in a column
11.3 [x] Add test for special characters in column names
11.4 [x] Add test for very large BigDecimal values (near precision limits)
