...
class MatrixAvroWriter {
  ...
  /**
   * Scans BigDecimal columns to infer the maximum precision and scale needed.
   *
   * <p>For each BigDecimal column, iterates through all rows to find:
   * <ul>
   *   <li>Maximum precision (total number of significant digits)</li>
   *   <li>Maximum scale (number of digits after decimal point)</li>
   * </ul>
   *
   * <p>Defaults to precision=10, scale=0 if column is empty or contains only nulls.
   *
   * @param matrix the Matrix to scan
   * @return a map from column name to [precision, scale] arrays for BigDecimal columns
   */
  ...
  /**
   * Determines if a Map column should be encoded as an Avro RECORD or MAP.
   *
   * <p>If all non-null Map values in the column have the same set of keys,
   * the column is treated as record-like (fixed schema). Otherwise, it's
   * treated as a free-form MAP with string keys.
   *
   * @param matrix the Matrix containing the column
   * @param col the column name
   * @return true if all maps have identical key sets (use RECORD), false otherwise (use MAP)
   */
  ...
  /**
   * Infers the element type for a List column by finding the first non-null element.
   *
   * <p>Scans through all List values in the column until a non-null element is found,
   * then returns that element's class. Falls back to String if no non-null elements exist.
   *
   * @param matrix the Matrix containing the column
   * @param col the column name
   * @return the Java class of list elements, or String as fallback
   */
  ...
  /**
   * Infers the value type for a Map column by finding the first non-null value.
   *
   * <p>Scans through all Map values in the column until a non-null map value is found,
   * then returns that value's class. Falls back to String if no non-null values exist.
   *
   * @param matrix the Matrix containing the column
   * @param col the column name
   * @return the Java class of map values, or String as fallback
   */
