...
class MatrixAvroRoundTripTest {
  ...
  void roundTrip_withDecimalInference_preservesTypes() {
    ...
    cols['price']     = [12.34, null, 1000.50] // BigDecimal
    ...
    assertBasicShape(src, back)
    assertStringAndNumericValues(back)
    assertDateTimeValues(back, t1, t2, i1, i2, ldt1, ldt2)
    assertDecimalAndBinaryValues(back, uuid1, uuid2)
    tmp.delete()
  }
  ...
  private static void assertStringAndNumericValues(Matrix back) {
  ...
  private static void assertDateTimeValues(Matrix back, LocalTime t1, LocalTime t2, Instant i1, Instant i2,
  ...
  private static void assertDecimalAndBinaryValues(Matrix back, UUID uuid1, UUID uuid2) {
    assertEquals(12.34, back[0, 'price'])
    assertNull(back[1, 'price'])
    assertEquals(1000.50, back[2, 'price'])
  ...
  void roundTripWithExplicitSchemaOverrides() {
    ...
    assertEquals(12.340, back[0, 'amount'])
    assertEquals(56.780, back[1, 'amount'])
  ...
  private static long rawLongFor(File avro, String field) {
    ...
    try {
      if (!rdr.hasNext()) {
        throw new IllegalStateException('no records written')
      }
      def rec = rdr.next()
