...
- `AvroSchemaDecl.bigInteger(precision)` is available for explicit lossless scale-zero decimal declarations. Existing `type(BigInteger)`, `arrayOf(BigInteger)`, and `mapOf(BigInteger)` calls remain supported and use the default precision of 10.
