...
- `Normalize` matrix-level helpers do not consistently select numeric columns by declared/nonnul numeric content; `minMaxNorm(List)`, `meanNorm(List)`, and `stdScaleNorm(List)` skip string columns based on `values[0]`, and matrix-level `logNorm(Matrix)` also lacks declared-type filtering. Transformed numeric columns can keep stale source type metadata.
