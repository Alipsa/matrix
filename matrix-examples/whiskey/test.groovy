#!/usr/bin/env groovy
@Grab(group='se.alipsa.matrix', module='matrix-bom', version='2.6.0', type='pom')
...
cacheDir.listFiles().eachWithIndex { it, idx ->
  println idx + ". " + it.name
  if (it.name.contains("matrix-core-3.9.0.jar")) {
    coreJarExists = true
  }
}
println "\nmatrix-core-3.9.0.jar was ${coreJarExists ? 'found' : 'NOT found'} in ${cacheDir.absolutePath}"
