...
- Improved `hypot()` last-place accuracy by calculating with DECIMAL128 guard precision and rounding every result to DECIMAL64, including zero-side short circuits. Zero-side inputs with more than 16 significant digits can therefore have different precision, scale, and `toString()` output.
- Improved `toDegrees()` and `toRadians()` accuracy by using the internal high-precision π and always rounding to DECIMAL64. Input-dependent noise digits are removed, so precision, scale, and `toString()` output can change.
