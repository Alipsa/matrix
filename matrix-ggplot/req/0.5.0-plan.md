...
5.1 [x] Add typed overloads for `guide_axis_stack` in `GgPlot.groovy`. The additional
...
5.2 [x] Replace the switch in `ScaleColorFermenter.resolvePaletteName()` in
...
5.3 [x] Add `acos(Number self)` and `acos(BigDecimal self)` to
...
Result: `acos()` implementation and tests were already present on the current branch before Phase 5 edits; verified with the matrix-groovy-ext command in 5.5.

5.4 [x] Add tests in `matrix-groovy-ext/src/test/groovy/se/alipsa/matrix/ext/NumberExtensionTest.groovy`
verifying:
...
5.5 [x] Verify the matrix-groovy-ext change with:
```
...
5.6 [x] Refactor the function-call branch in `convertExpression` (around line 386) in
...
5.7 [x] For the `pi` token (line 377), emit the fully-qualified
...
5.8 [x] Add or update tests in `matrix-ggplot/src/test/groovy/gg/RconverterTest.groovy`
...
5.9 [x] Verify Phase 5 with:
```
./gradlew :matrix-ggplot:codenarcMain :matrix-ggplot:codenarcTest :matrix-ggplot:spotlessCheck :matrix-ggplot:test -Pheadless=true
```

Result: SUCCESS (1780 tests, 1780 passed, 0 failed, 0 skipped).
...
X.1 [x] Run the full test suite for all modules touched by this release (Phase 5c modifies
...
Result: SUCCESS (matrix-ggplot: 1790 tests, 1790 passed, 0 failed, 0 skipped; matrix-groovy-ext test task up-to-date).

X.2 [x] Confirm `build/reports/codenarc/main.html` and `build/reports/codenarc/test.html`
report zero violations for **both** `matrix-groovy-ext` and `matrix-ggplot`.

Result: Confirmed with:
```
rg -o "<PackageSummary[^>]+>" matrix-groovy-ext/build/reports/codenarc/main.xml matrix-groovy-ext/build/reports/codenarc/test.xml matrix-ggplot/build/reports/codenarc/main.xml matrix-ggplot/build/reports/codenarc/test.xml
```
All four reports showed `filesWithViolations='0'`, `priority1='0'`, `priority2='0'`, and
`priority3='0'`.
