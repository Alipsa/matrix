...
| Phase Task 5.9.1: Implement FacetWrap parity in charm, including free scales (`fixed`, `free`, `free_x`, `free_y`), `ncol`, `nrow`, `dir`, multi-variable composite keys, labeller support (free scales not implemented; panels share fixed scales — tracked in `matrix-ggplot/req/v0.6.0-fixes.md` §16). | Cross-cutting | `charm/render/FacetRenderer`, `charm/render/CharmRenderer` | `CharmFacetThemeTest` | [ ] |
